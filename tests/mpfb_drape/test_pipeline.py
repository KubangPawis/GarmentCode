...
def test_drape_one_injects_seg_and_returns_record(tmp_path):
    ...
    # Staging into assets/bodies is what PathCofig requires (it reads the body yaml
    # from there at construction).
    bodies = REPO / "assets/bodies"
    body_yaml = bodies / "mean_all_tpose.yaml"
    body_obj = bodies / "mean_all_tpose.obj"
    seg_extra = bodies / "mean_all_tpose_bodyseg.json"  # generated by staging; cleaned up below
    ...
    try:
        ...
        assert "mean_all_tpose_bodyseg.json" in captured["body_seg"]
...
def test_drape_one_rejects_arms_down_body(tmp_path):
    ...
    # NOT a cryptic deep Warp crash.
    bodies = REPO / "assets/bodies"
    seg_extra = bodies / "mean_all_bodyseg.json"
    with open(REPO / "assets/design_params/t-shirt.yaml") as f:
        design = yaml.safe_load(f)["design"]
    try:
        with pytest.raises(ValueError, match="true T-pose"):
            pipeline.drape_one(
                ...
                out_dir=tmp_path / "out", bodies_dir=bodies, name="tee",
